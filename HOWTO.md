...
1. チェックイン専用ページ（`ticket-checkin.fwj.jp`）にアクセス
...
1. ナビゲーションバーの「Shopify検索」→「FWJ会員検索」をクリック
2. 検索ボックスに以下のいずれかを入力：
   - メールアドレス
   - 氏名（日本語・英語）
   - Shopify ID
...
1. ナビゲーションバーの「Shopify検索」→「注文検索」をクリック
2. 「注文タグ」欄に検索したいタグを入力
   - 複数タグはスペース区切りでAND検索
