...
const tickets = pgTable('tickets', {
  ...
  productId: varchar('product_id', { length: 50 }).notNull().default(''),
  ...
  tag3: varchar('tag3', { length: 255 }),
  tag4: varchar('tag4', { length: 255 }),
  tag5: varchar('tag5', { length: 255 }),
  tag6: varchar('tag6', { length: 255 }),
  tag7: varchar('tag7', { length: 255 }),
  tag8: varchar('tag8', { length: 255 }),
  tag9: varchar('tag9', { length: 255 }),
  tag10: varchar('tag10', { length: 255 }),
...
const orders = pgTable('orders', {
  ...
  productId: varchar('product_id', { length: 50 }).default(''),
  backStagePass: integer('back_stage_pass').default(0),
...
const judges = pgTable('judges', {
  id: serial('id').primaryKey(),
  contestName: varchar('contest_name', { length: 255 }).notNull(),
  contestDate: varchar('contest_date', { length: 20 }),
  className: varchar('class_name', { length: 255 }).notNull(),
  playerNo: integer('player_no').notNull(),
  playerName: varchar('player_name', { length: 255 }),
  placing: integer('placing').notNull(),
  scoreJ1: integer('score_j1'),
  scoreJ2: integer('score_j2'),
  scoreJ3: integer('score_j3'),
  scoreJ4: integer('score_j4'),
  scoreJ5: integer('score_j5'),
  scoreT: integer('score_t'),
  isValid: boolean('is_valid').notNull().default(true),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow(),
}, (table) => [
  index('idx_judges_contest_name').on(table.contestName),
  index('idx_judges_class_name').on(table.className),
  index('idx_judges_is_valid').on(table.isValid),
]);
...
const pushSubscriptions = pgTable('push_subscriptions', {
  ...
  keyP256dh: varchar('key_p256dh', { length: 512 }).notNull(),
  keyAuth: varchar('key_auth', { length: 512 }).notNull(),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow(),
}, (table) => [
  index('idx_push_subscriptions_shopify_id').on(table.shopifyId),
  unique('unique_push_endpoint').on(table.endpoint),
]);

module.exports = { tickets, contests, guests, users, subjects, notes, scores, judges, registrations, members, orders, orderExportMeta, pushSubscriptions };
